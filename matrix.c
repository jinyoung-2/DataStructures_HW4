...
void print_matrix(int** matrix, int row, int col)
{
    ...
    if (row <= 0 || col <= 0) {
        ...
        for (int i = 0; i < row; i++) {
            for (int j = 0; j < col; j++)
                printf("%4d", matrix[i][j]);    //matrix[i][j]를 이중for문을 통해 출력         
            printf("\n");
        }
        printf("\n");
    }
...
int addition_matrix(int** matrix_a, int** matrix_b, int row, int col)
{
    ...
    for (int i = 0; i < row; i++) {
        for (int j = 0; j < col; j++) {
            /* check post conditions */
            //값이 0~19로 한정되어 있어, 더한 값이 0보다 작을 수 없고 38보다 클 수 없다.
            if ((matrix_a[i][j] + matrix_b[i][j] < 0) || (matrix_a[i][j] + matrix_b[i][j] > 38)) {      
...
int subtraction_matrix(int** matrix_a, int** matrix_b, int row, int col)
{
    ...
    for (int i = 0; i < row; i++) {
        for (int j = 0; j < col; j++) {
            /* check post conditions */
            //값이 0~19로 한정되어 있어, 뺀 값이 -19보다 작을 수 없고 19보다 클 수 없다.
            if ((matrix_a[i][j] - matrix_b[i][j] > 19) || (matrix_a[i][j] - matrix_b[i][j] < -19)) {       
...
int transpose_matrix(int** matrix, int** matrix_t, int row, int col)
{
    ...
    for (int i = 0; i < row; i++) {
        for (int j = 0; j < col; j++) {
            matrix_t[i][j] = matrix[j][i];                          /*전치행렬은 기존 행렬의 행과 열이 바뀐 행렬이다 행과 열의 인덱스를 바꿔 matrix_t에 값을 대입한다.*/
            /* check post conditions */
    ...
    for (int i = 0; i < row1; i++){                                  /* 행렬의 곱인 matrix_c의 행만큼 반복*/
        for (int j = 0; j < col2; j++){  //col2=row1                 /* 행렬의 곱인 matrix_c의 열만큼 반복*/
            ...
            for (int k = 0; k < row2; k++){  //row2=col1             /* 행렬 matrix_a의 열, matrix_t의 행만큼 반복*/
                /* check post conditions */
                //두 행렬의 값이 0~19로, 곱이 0보다 작을 수 없고 361보다 클 수 없다.
                if ((matrix_a[i][k] * matrix_t[k][j] < 0) || (matrix_a[i][k] * matrix_t[k][j] > 361)){  
